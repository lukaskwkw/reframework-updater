...
impl ReleaseREFR {
    pub fn from_release(release: &serde_json::Value) -> DynResult<Release> {
        let tag = release["tag_name"]
            .as_str()
            .ok_or_else(|| "Release missing `tag_name`")?;
        let date = release["created_at"]
            .as_str()
            .ok_or_else(|| "Release missing `created_at`")?;
        let name = release["name"].as_str().unwrap_or(tag);
        let assets = release["assets"]
            .as_array()
            .ok_or_else(|| "No assets found")?;
    ...
    fn from_asset(asset: &serde_json::Value) -> DynResult<ReleaseAsset> {
        let download_url = asset["url"]
            .as_str()
            .ok_or_else(|| "Asset missing `url`")?;
        let name = asset["name"]
            .as_str()
            .ok_or_else(|| "Asset missing `name`")?;
        Ok(ReleaseAsset {
